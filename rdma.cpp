...
/*
* =======================================================================
* NOTICE: This file was fully generated by generative AI based on
* a natural language prompt or task description.
*
* It has been launched for basic checks but not thoroughly reviewed for
* safety, correctness, or completeness. Use at your own risk.
*
* The code is provided as-is without warranty or guarantee of accuracy.
* The AI does not run or test the code and may introduce errors or omissions.
*
* You are responsible for reviewing, testing, and adapting this code to
* your needs. Please use caution and best practices before integration.
* =======================================================================
*/
...
public:
    ...
    void Run(const char* localAddr) {
        ...
        // Post a receive for the client's message
        ND2_SGE sge = { m_Buf, TEST_BUFFER_SIZE, m_pMr->GetLocalToken() };
        if (FAILED(PostReceive(&sge, 1, (void*)1))) {
            std::cerr << "PostReceive failed." << std::endl;
            return;
        }

        if (!WaitForCompletionAndCheckContext((void*)1)) {
        ...
        if (FAILED(Send(&sge, 1, 0, (void*)2))) {
            std::cerr << "Send failed." << std::endl;
            return;
        }

        if (!WaitForCompletionAndCheckContext((void*)2)) {
...
public:
    ...
    void Run(const char* localAddr, const char* serverAddr) {
        ...
        ND2_SGE sge = { m_Buf, (ULONG)strlen(message) + 1, m_pMr->GetLocalToken() };

        if (FAILED(Send(&sge, 1, 0, (void*)3))) {
            std::cerr << "Send failed." << std::endl;
            return;
        }

        if (!WaitForCompletionAndCheckContext((void*)3)) {
            std::cerr << "WaitForCompletion for send failed." << std::endl;
            return;
        }
        ...
        if (FAILED(PostReceive(&sge, 1, (void*)4))) {
            std::cerr << "PostReceive failed." << std::endl;
            return;
        }

        if (!WaitForCompletionAndCheckContext((void*)4)) {
            std::cerr << "WaitForCompletion for receive failed." << std::endl;
            return;
        }
